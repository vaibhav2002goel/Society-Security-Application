...
<title>Regular Visitor's Log</title>
	...
	<div class="bg">
		...
		<table class="table table-striped" style="width:80%; margin:auto; ">
		  <thead>
		    <tr class="table-success">
		      ...
		      <th scope="col">ENTER CHECK-OUT TIME</th>
		    </tr>
		  </thead>
	    ...
	    <%
			...
			try(Connection con = DriverManager.getConnection(url,user,password)) {
					...
					String query1 = "select * from regularVisitor where supervisorUsername=? ";
		...
		%>	
					    <tr>
					      ...
					      <td><%= rs1.getString("securityCode") %></td>	
					      <td><%= rs1.getInt("flatNumber") %></td>	
					      <td><%= rs1.getInt("buildingNumber") %></td>
					      <td><%= rs1.getString("checkInTime") %></td>	
					      <td><%= rs1.getString("checkOutTime") %></td>
					      
					      <td> 
						      <form action="regularVisitorCheckOutTime?code=<%= rs1.getString("securityCode") %>&username=<%= username %>" method="post">				      	
						      	<input type="time" name="checkOutTime" placeholder="Enter Check-Out Time" required>
