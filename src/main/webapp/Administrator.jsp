...
<%-- 		<h1> <%= (String) session.getAttribute("firstName") %> <%= (String) session.getAttribute("lastName") %> </h1> --%>
	    ...
	    <%!
			...
			String query1 = "select * from FLAT where flatAdmin=? ";
	    %>
		...
		%>
					...
					<div style="margin:0 1vw;" class="card w-25 text-bg-light mb-3 border-secondary " style="width: 18rem;">
					  ...
					  <div class="card-body">
					    <h5 class="card-title"><%= rs1.getString("allocatedPersonUsername") %></h5>
					    <p class="card-text"> FLAT NUMBER : <%= rs1.getInt("flatNumber") %></p>
					  </div>
					  <ul class="list-group list-group-flush">
					    <li class="list-group-item">BUILDING NUMBER : <b><%= rs1.getInt("flatBuildingNumber") %></b></li>
					    <li class="list-group-item">BHK : <b><%= rs1.getString("BHK") %></b></li>
					    <li class="list-group-item">MAINTENANCE CHARGES : <b><%= rs1.getDouble("maintenanceCharges") %></b> </li>
					    <li class="list-group-item">FLAT PRICE : <b><%= rs1.getDouble("costOfBuying") %></b> </li>
					  </ul>
					  <div class="card-body">
					  <p class="card-link"> FLAT ADMINSTRATOR : <b><%= rs1.getString("flatAdmin") %></b>  </p>
					  </div>	  
		       		</div>
